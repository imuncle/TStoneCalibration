...
void HandEyeCalibration::calibrate()
{
    ...
    std::vector<cv::Mat> cTo;
    cv::Mat cRo_i, cPo_i;
    cv::Mat cTc = cv::Mat::zeros(3, 3, CV_64F);
    cTc.at<double>(0, 1) = +1;
    cTc.at<double>(1, 0) = +1;
    cTc.at<double>(2, 2) = -1;
    if (!ui->double_camera->isChecked()) //
    {
        for(unsigned int i = 0; i < imgs.size(); i++)
        {
            ...
            {
                cPo_i = imgs[i].tvec;
                cv::Rodrigues(imgs[i].rvec, cRo_i);
            }
            else
            {
                cPo_i = imgs[i].fish_tvec;
                cv::Rodrigues(imgs[i].fish_rvec, cRo_i);
            }

            cv::Mat cTo_i = cv::Mat::zeros(cv::Size(4, 4), CV_64F);
            for (int r = 0; r < 3; ++ r) {
                for (int c = 0; c < 3; ++ c) {
                    cTo_i.at<double>(r, c) = cRo_i.at<double>(r, c);
                }
                cTo_i.at<double>(r, 3) = cPo_i.at<double>(r, 0) / 1000.0;
            }
            cTo_i.at<double>(3, 3) = 1;
            cTo.push_back(cTo_i);
        }
    } else {
        for(unsigned int i = 0; i < stereo_imgs.size(); i++)
        {
            if(fisheye_flag == false)
            {
                cPo_i = stereo_imgs[i].left_tvec;
                cv::Rodrigues(stereo_imgs[i].left_rvec, cRo_i);
            }
            else
            {
                cPo_i = stereo_imgs[i].left_fish_tvec;
                cv::Rodrigues(stereo_imgs[i].left_fish_rvec, cRo_i);
            }
    ...
    TSAIleastSquareCalibration::runCalibration(robot_poses, cTo, ui->k_eye_in_hand->isChecked(), extrinsicMatrix);

    calibrate_flag = true;
}

// 导出标定参数
void HandEyeCalibration::exportParam()
{
    if(calibrate_flag == false)
    {
    ...
    fs_write << "extrinsicMatrix" << extrinsicMatrix;
...
// 单目相机图片加载，逻辑同双目相机图片加载
void HandEyeCalibration::addRobot()
{
    robot_poses.clear();
    QStringList path_list = QFileDialog::getOpenFileNames(this, tr("选择文件"), tr("./"), tr("文本文件(*.txt);;所有文件(*.*);"));
    for(int i = 0; i < path_list.size(); i++)
    {
        cv::Mat robot_pose = cv::Mat::zeros(cv::Size(4, 4), CV_64F);
        QFileInfo file = QFileInfo(path_list[i]);
        QString file_name = file.fileName();
        std::ifstream in(path_list[i].toStdString());
        if (in.is_open())
        {
            in >> robot_pose.at<double>(0, 0) >> robot_pose.at<double>(0, 1) >> robot_pose.at<double>(0, 2) >> robot_pose.at<double>(0, 3) \
               >> robot_pose.at<double>(1, 0) >> robot_pose.at<double>(1, 1) >> robot_pose.at<double>(1, 2) >> robot_pose.at<double>(1, 3) \
               >> robot_pose.at<double>(2, 0) >> robot_pose.at<double>(2, 1) >> robot_pose.at<double>(2, 2) >> robot_pose.at<double>(2, 3) \
               >> robot_pose.at<double>(3, 0) >> robot_pose.at<double>(3, 1) >> robot_pose.at<double>(3, 2) >> robot_pose.at<double>(3, 3);
        }
        in.close();
